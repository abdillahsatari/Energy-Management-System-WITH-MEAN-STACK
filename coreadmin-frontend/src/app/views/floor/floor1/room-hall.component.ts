...
@Component({
  templateUrl: 'room-hall.component.html'
})
export class RoomHallComponent implements OnInit, OnDestroy {
  ...
  fetchDataItem(){
    ...
    let content3 = document.getElementById("ac");
     ...
     this._activeRoute.params.subscribe(params => {
      ...
      //params.lantai mengacu pada atau harus sama dengan path pada floor-routing module.

      this._dataService.getRoomById(this.lantai).subscribe(res => {
        this.datastatus = res;
        //res mengacu ke res.json(room) yang ada pada datahandler /fetchRoom/:room
      ...
      if (this.datastatus[0].status == "1"){
          content1.setAttribute('class','activeLamp1');
